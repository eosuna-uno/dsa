export type PriorityQueueValue<T> = {
  val: T;
  priority: number;
};
export class PriorityQueue<T> {
  values: PriorityQueueValue<T>[] = [];
  constructor() {}
  public enqueue(priority: number, val: T) {
  ...
  private return_idx_max_value(
    ...
    let max = this.values[index].priority;
    if (child_1 === null && child_2 === null) return index; //finish
    if (child_1) {
      max = Math.min(max, this.values[child_1].priority);
    }
    if (child_2) max = Math.min(max, this.values[child_2].priority);
    if (child_1 && this.values[child_1].priority === max) {
      return child_1;
    }
    if (child_2 && this.values[child_2].priority === max) {
      return child_2;
    }
    return index;
  }
  ...
  buble_up(bubble_index: number) {
    ...
    while (
      ...
      this.values[bubble_index] &&
      this.values[parent_index].priority > this.values[bubble_index].priority
    ) {
